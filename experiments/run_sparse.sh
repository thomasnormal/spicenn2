...
env $B DATAFILE=digits_train.npz SEED=1 python3 "$SPICENN_ROOT/experiments/gen_mc.py" $SLOTS 0.3 1.5e-3 >/dev/null 2>&1
echo "MOS=$(grep -c '^M' mc.cir) weights=$(($K*10+10))"
T0=$(date +%s); ngspice -b mc.cir >/dev/null 2>&1; echo "train $(($(date +%s)-T0))s"
env $B DATAFILE_TE=digits_test.npz AVGW=20 python3 "$SPICENN_ROOT/experiments/gen_mc_infer.py" >/dev/null 2>&1; ngspice -b mc_infer.cir >/dev/null 2>&1
printf "SPARSE N=$N K=$K: "; C=10 python3 "$SPICENN_ROOT/experiments/score_mc.py" "" 2>&1
