...
for it in $(seq 1 80); do
  ...
  env $B TAG=$T $IC DATAFILE=digits_train.npz SEED=1 python3 gen_mc.py $PER 0.3 1.5e-3 >/dev/null 2>&1 || continue
  ...
  if [ $((it % 3)) -eq 0 ] && [ -s ckpt.txt ]; then
    env $B TAG=$T WFILE=ckpt.txt ICW=ckpt.txt DATAFILE_TE=digits_test.npz AVGW=20 python3 gen_mc_infer.py >/dev/null 2>&1
    ngspice -b mc_infer$T.cir >/dev/null 2>&1
    printf "@%s slots: " "$cum">>ckpt_log.txt; TAG=$T C=10 python3 score_mc.py "">>ckpt_log.txt 2>&1
  fi
done
