...
python3 "$SPICENN_ROOT/experiments/prep_2d.py" $TASK ${NTR:-50} ${NTE:-60} ${NVAL:-30}
...
if [ "${SIM:-ngspice}" = "spectre" ]; then
  ln -sf "$SPICENN_ROOT/experiments/spectre_mc.py" spectre_mc.py 2>/dev/null
  AVGW=20 python3 "$SPICENN_ROOT/experiments/spectre_mc.py" train
else
...
if [ "${SIM:-ngspice}" = "spectre" ]; then
  python3 "$SPICENN_ROOT/experiments/spectre_mc.py" infer
else
  timeout 400 ngspice -b mc_infer.cir >/dev/null 2>&1
fi
C=2 python3 "$SPICENN_ROOT/experiments/score_mc.py" "BACKPROP-2D $TASK (2->${H:-16}->2) seed=${SEED:-5} ${SIM:-ngspice}"
