...
for it in $(seq 1 40); do
  ...
  env $B $IC DATAFILE=digits_train.npz SEED=1 python3 gen_mc.py $PER 0.3 $CG >/dev/null 2>&1 || continue
  ...
  if [ $((it % 4)) -eq 0 ]; then
    env $B ICW=ckpt.txt DATAFILE_TE=digits_test.npz AVGW=20 python3 gen_mc_infer.py >/dev/null 2>&1
    ngspice -b mc_infer.cir >/dev/null 2>&1
    printf "  @%s slots: " "$cum" >> ckpt_log.txt; C=10 python3 score_mc.py "" >> ckpt_log.txt 2>&1
  fi
done
