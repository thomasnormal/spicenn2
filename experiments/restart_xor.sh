...
for task in 1 2 3 4 5 6; do
  ...
  for a in $(seq 1 $MAXTRY); do
    S=$((task*100+a))
    env $base SEED=$S python3 "$SPICENN_ROOT/experiments/gen_xor_full.py" 1000 0.3 5e-4 >/dev/null 2>&1
    timeout 200 ngspice -b xor_full.cir >/dev/null 2>&1
    env $base SEED=$S AVGW=40 python3 "$SPICENN_ROOT/experiments/gen_infer.py" >/dev/null 2>&1
    timeout 60 ngspice -b xor_infer.cir >/dev/null 2>&1
    r=$(python3 "$SPICENN_ROOT/experiments/score.py" "")
