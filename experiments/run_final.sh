...
score () { export C; TESTSEED=2 NTE=20 AVGW=15 python3 "$SPICENN_ROOT/experiments/gen_mc_infer.py" >/dev/null 2>&1; timeout 90 ngspice -b mc_infer.cir >/dev/null 2>&1; C=$C python3 "$SPICENN_ROOT/experiments/score_mc.py" "" 2>&1; }
for C in 3 4 6; do
  for COMP in none subtractive; do
    for SEED in 1 7; do
      export C COMP SEED
      python3 "$SPICENN_ROOT/experiments/gen_mc.py" 1400 0.3 5e-4 >/dev/null 2>&1
