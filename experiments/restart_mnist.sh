...
NORM=perimage python3 "$SPICENN_ROOT/experiments/prep_mnist.py" $LABELS ${NTR:-24} ${NTE:-50} ${NVAL:-30}
best=-1; bestatt=0
for a in $(seq 1 $K); do
  env $BCFG $TR SEED=$a NSEED=$a python3 "$SPICENN_ROOT/experiments/gen_mc.py" $SLOTS 0.3 5e-4 >/dev/null 2>&1
  timeout 250 ngspice -b mc.cir >/dev/null 2>&1
  env $BCFG DATAFILE_TE=digits_val.npz AVGW=20 python3 "$SPICENN_ROOT/experiments/gen_mc_infer.py" >/dev/null 2>&1
  timeout 120 ngspice -b mc_infer.cir >/dev/null 2>&1
  v=$(C=$NC python3 "$SPICENN_ROOT/experiments/valscore.py" mc_test_labels.npy)
...
env $BCFG DATAFILE_TE=digits_test.npz AVGW=20 WFILE=mc_weights_best.txt python3 "$SPICENN_ROOT/experiments/gen_mc_infer.py" >/dev/null 2>&1
timeout 120 ngspice -b mc_infer.cir >/dev/null 2>&1
