...
print('N=%d ideal=%.1f%%'%(N,100*LogisticRegression(max_iter=400).fit((Xtr+1)/2,Ytr).score((Xte+1)/2,Yte)))
PY
...
env $B DATAFILE=digits_train.npz SEED=${6:-1} python3 "$SPICENN_ROOT/experiments/gen_mc.py" $SLOTS 0.3 1.5e-3 >/dev/null
