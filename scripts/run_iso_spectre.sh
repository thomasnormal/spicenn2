...
cp "$SPICENN_ROOT/experiments/gen_mc.py" "$SPICENN_ROOT/experiments/gen_mc_infer.py" "$SPICENN_ROOT/experiments/score_mc.py" "$SPICENN_ROOT/experiments/spectre_mc.py" "$SPICENN_ROOT/mnist_8x8.npz" .
...
T0=$(date +%s); MT=4 python3 "$SPICENN_ROOT/experiments/spectre_mc.py" train >/dev/null 2>&1; echo "train $(($(date +%s)-T0))s"
env $B DATAFILE_TE=digits_test.npz AVGW=20 python3 "$SPICENN_ROOT/experiments/gen_mc_infer.py" >/dev/null; MT=4 python3 "$SPICENN_ROOT/experiments/spectre_mc.py" infer >/dev/null 2>&1
printf "SPECTRE N=%s: " "$N"; C=10 python3 "$SPICENN_ROOT/experiments/score_mc.py" ""
