...
10-class, 64--48 random hidden (no auto-zero; baseline) & $\sim$5{,}507 & 180 & 80 & 0.587 \\
\bottomrule
\end{tabular}
...
The in-circuit learner sits within $\sim$3 points of the backprop ideal at this scale. Two sizing laws emerged: readout fan-in should be $\approx C$, and the readout must cover $\gtrsim 20\%$ of the feature pool (without per-class auto-zeroing, 48 features beat 96 at fan-in 10: $0.587$ vs $0.433$). \emph{Adding per-class auto-zeroing changes this picture decisively}: each class's output is high-pass filtered against a slow running mean of its own activity, removing the systematic per-class offset that otherwise drives a rich-get-richer output collapse. With it, the feature-count ordering inverts---more features now help---and the 10-class result rises to $\mathbf{0.747}$ at 96 features (ideal $0.853$, all ten classes alive; consistent across initialization seeds at $74.7/74.8/74.7\%$), the best many-class result here. Beyond ${\sim}96$ random features the task saturates (the $8{\times}8$ digit input is information-limited: $256$ features give a \emph{lower} ideal, $0.833$).

\paragraph{The 10-class ceiling is operating-point-saturated, not under-tuned.} We stress-tested this $\sim$0.747 result against an exhaustive sweep of the learning rule's free parameters and find it a hard analog ceiling rather than a missed optimum. With the architecture fixed (fixed random features into a dense readout, the strongest known configuration here), the number does not move under: doubling the per-slot settling time; linearizing the readout synapse (more source degeneration \emph{monotonically} hurts---$0.752\!\to\!0.724\!\to\!0.692$ as degeneration rises---the small-signal gain loss outweighs any fixed-point benefit); raising neuron gain (a $2\times$-wider differential pair is worse, more tail current is neutral---null-to-negative here as in the depth experiments); structural cell changes (a cascode output stage and a $2\times$ load resistance both tie the baseline within 3-seed noise, $+0.005$ and $0.000$; a quartic-nonlinearity neuron collapses---tanh stays the digit neuron); or trading training epochs for more data at a fixed simulation budget. Across this sweep the circuit holds $0.744$--$0.752$ (3-seed, bijective-permutation scored), so the residual to the matched ideal ($0.747$ vs $0.853$, ${\sim}89\%$) is device-cell transfer fidelity---the analog neuron is a real but imperfect $\tanh$---placing the next lever at the cell level, not the algorithm. Two intermediate explanations for the saturation that we entertained, readout summing noise and incomplete settling, were tested and rejected; the surrogate-vs-silicon divergence behind them is itself the caution of Section~\ref{sec:limits}.

\paragraph{The hidden layers learn at ten classes.} The $0.747$ above is the strongest \emph{absolute} ten-class number, but it freezes the hidden layer---random features into a trained readout---and so does not itself show deep hidden weights learning at this class count. They do. On the rule-respecting deep pyramid (64--36--16, fan-in $\le 7$, local receptive fields), training the hidden layers in circuit beats freezing them at matched architecture: trained $0.496\pm0.025$ versus frozen $0.429\pm0.026$ (3-seed, bijective-permutation scored, epoch 8), a gap whose seed distributions do not overlap (trained minimum $0.464 >$ frozen maximum $0.448$). The trained accuracy rises with training ($0.50\!\to\!0.516$ by epoch 12) while the frozen control is flat ($0.448\!\to\!0.452$)---a learning curve, not a fixed offset. The effect is robust to the backward implementation: a graded signed-transpose backward (three seeds, $0.496$) and a hard-sign amplitude-restoring backward (single seed, $0.512$, inside the graded spread) reach the same accuracy, so the enabler is functioning error transport through depth, not the particular backward cell---and at this depth (three stages) the graded path retains enough amplitude to work unaided, with the re-amplification of Section~\ref{sec:bksign} reserved for deeper nets. The trained deep network sits below the frozen-wide shortcut in absolute terms---a few sequential features cannot yet match many parallel ones into a dense readout---but it is the result that matters for the central claim: at ten classes the hidden layers of a deep analog network learn from locally transported error, with no weight transport, no frozen substructure, and fan-in $\le 7$.

\subsection{Depth and amplitude-restoring error transport}
\label{sec:bksign}

Deep sparse pyramids (64--32--16--8--4, fan-in 4) plateau at $0.55$ under joint training, far below the depth-2 reference ($0.890$). The natural assumption is that the chained analog transposes corrupt the error \emph{direction}; we find instead that the dominant failure is loss of error \emph{magnitude}. Each analog stage attenuates the backward signal (output-conductance compression, finite gain), so after several stages the error is a tiny fraction of full scale and the deep layers receive no usable drive---while the direction it still carries remains good enough to learn from (a backward path that is cosine-imperfect but full-amplitude trains to $0.94$; the same path attenuated $3\times$ collapses to $0.68$).

The repair is therefore cheap re-amplification rather than faithful magnitude transport: at each layer a high-gain saturating cell (\texttt{dneuron} as a soft comparator, $0.537\tanh(15.7\,u)$) restores the attenuated error toward full swing. For the small errors typical after attenuation it is near-linear---it preserves the sign \emph{and} the magnitude, merely undoing the amplitude loss; only large errors saturate toward a pure sign. So this is amplitude restoration, not magnitude destruction. The regeneration drive (how hard the cell saturates) has a tuned optimum at the operating point we use: sweeping it noise-free on the depth-4 net, both extremes are worse---driving toward the near-linear, magnitude-preserving regime ($0.73$) and toward a hard sign ($0.76$) each underperform the default ($0.83$). So the system is not improved by pushing toward either pure magnitude or pure sign; the value is restoring the attenuated \emph{amplitude}, with the residual nonlinearity neither helping nor hurting much once amplitude is recovered. (Consistent with this, the weight \emph{update} tolerates sign-only---the sign-update cell matches the full multiply below.) Per hidden neuron we add (Fig.~\ref{fig:bksign}): a collector wire summing the backward synapse currents; a \texttt{dneuron} used as a \emph{comparator} (gain ${\approx}16$, saturating) that regenerates the collected error to full swing; and a fixed-negative-unity \texttt{gsyn} injecting the result into the value node. Cost: ${\sim}14$ transistors per hidden neuron, no capacitors.
...
\draw[sig] (inj) -- (x);
\end{tikzpicture}
\caption{\textbf{Amplitude-restoring backward (BKSIGN)}, one hidden neuron $j$ of layer $\ell$. The children's errors are weighted by the (shared) weight capacitors, summed as currents, \emph{re-amplified toward full swing} by a high-gain saturating cell (preserving sign, and magnitude up to saturation), and injected into the value node. This undoes the per-stage amplitude attenuation that otherwise starves deep layers of drive.}
\label{fig:bksign}
\end{figure}
...
\textbf{Joint, amplitude-restoring backward} & \textbf{0.84} \\
...
\bottomrule
\end{tabular}
\caption{Depth results (digits, $C{=}4$). The amplitude-restoring backward (BKSIGN) nearly closes the depth gap, renders the curriculum unnecessary (and counterproductive), is neutral at depth 2, and degrades gracefully at depth 6.}
\label{tab:depth}
\end{table}
...
\paragraph{Learnable biases (intrinsic plasticity).} The networks above have no bias parameters --- an omission with consequences for both expressivity (a no-bias tanh can only place its transition at the input common mode) and offset absorption. Adding one per neuron --- a weight-capacitor pair driven by a constant unit input, updated by its own $\eps\cdot 1$ cell --- sets a new clean depth-4 record: \textbf{0.860} (vs.\ 0.840), reached as a stable plateau with \emph{no late-training drift}, suggesting part of the drift in bias-free networks was bias error being forced into weights that cannot express it. Under mismatch, biases are the single strongest self-calibration lever (below).

\paragraph{Dale's law.} Biological neurons have a fixed sign --- excitatory or inhibitory --- while artificial synapses flip sign freely. We tested the constraint: each hidden neuron is randomly assigned a fixed sign (50/50), inhibitory neurons' outgoing synapses are wired with crossed outputs (sign flips are free in differential signaling), the transpose and update paths are crossed to match, and all weights start positive. The constrained network reaches a best epoch of \textbf{0.80} versus 0.84 unconstrained --- the biological sign discipline costs only $\sim$4 points, suggesting single-sign synapse populations (which admit far cheaper single-ended cells) are a viable direction. Late-training drift (0.80 to 0.62 final) indicates magnitude-only plasticity wants an explicit $w \ge 0$ enforcement or a controller-timed freeze.
...
\subsection{A high-gain rectifier neuron (\texttt{nrelu}, 8T+2R)}
\label{sec:nrelu}

The tanh neuron above is what a differential pair gives for free; a \emph{rectifier} is what a single transistor gives for free. The \texttt{nrelu} cell is a one-sided V-to-I leg (transistor in cutoff below the knee at $v_{rl}+V_T$, degenerated by $12$\,k$\Omega$ above it) mirrored onto a resistively loaded output, with a matched reference leg on the other output for a defined differential zero. Three findings turn this from a curiosity into the strongest neuron in the library --- all from minutes-scale screening runs (small weight capacitors, $32$-point test sets; we label these Q-tier throughout):

\begin{itemize}
\item \textbf{Polarity must be upright by construction.} The cell inverts (the signal leg mirrors onto the negative output). At low gain, learnable biases can flip an inverted network into the correct solution mid-training; at higher gain they cannot, and the network converges to \emph{perfect anti-classification} (accuracy $\approx 0$, not $0.5$). Swapping the output pins at instantiation --- free in differential wiring --- removes the failure class.
\item \textbf{Gain is the unlock, and it obeys a width law.} The PMOS mirror ratio sets the activation gain. At gain ${\sim}1$ the rectifier network is no better than tanh; at gain ${\sim}5$ ($4\times$ mirror) a tiny 2--4--2 network solves \emph{all three} 2D benchmarks --- circles, rings, and the two-spirals problem that defeats single-hidden-layer tanh networks. At final length (64 epochs, 256-point test sets, learning rate annealed to zero at the measured peak) the triple reads $\mathbf{0.992 / 0.988 / 0.992}$ (circles/rings/spirals), each \emph{flat} across the full horizon --- the strongest 2D numbers of the project, from one net and plain local learning. The required per-cell gain follows a \emph{cascade-gain budget} (a measured design law, Section~\ref{sec:laws}): we mapped the train/anti-lock boundary on a gain grid for one-, two-, and three-hidden-layer networks of fixed width, and the total cascade gain---roughly the per-stage gain raised to the depth---must sit in a bounded band. A single-hidden-layer net needs the gain \emph{cranked} (it trains only at the $4\times$ mirror and above, never over-driving); adding a layer both \emph{lowers} the gain it needs and introduces a \emph{ceiling}---a two-hidden-layer net trains across a wide low-gain band but anti-locks at the highest gain the shallow net required. So per-stage gain should scale as $G_*^{1/\text{depth}}$: turn it down as you go deeper. Gain near the ceiling solves and then flips basins mid-training unless the learning rate is annealed.
\item \textbf{Biases are mandatory, and failures are detectable.} Without per-neuron biases the rectifier network sits exactly at chance (the knee cannot place itself). The residual failure mode is an init-time ``anti-lock'' ($\sim$25\% of weight draws): the network trains to the label-swapped solution, immediately visible as training accuracy near zero, and a weight re-roll fixes it (we provide a three-try restart wrapper; the law of small training sets applies to the rest).
\end{itemize}

Per-device offset sensitivity is identical to the tanh neuron's --- a gate $\Delta V_T$ shifts either transfer curve by exactly $\Delta V_T$ input-referred (DC characterization) --- so the rectifier buys expressivity, not mismatch immunity; its mismatch story rides on the same bias-absorption mechanism as everything else. The cell composes with the rest of the library unchanged: under Dale's-law sign constraints it holds $0.97$, and trained by the 9-transistor sign-update cell it holds $0.97$. The division of labor between the two neurons is empirical and clean: the rectifier wins every 2D geometry benchmark; the tanh pair remains stronger on the 64-input digits task at every gain and layer placement we tried --- we report both and use each where it wins.
...
The dose-response (Table~\ref{tab:mismatch}) is sobering, and a three-chip Monte-Carlo at $\sigma{=}2$\,mV shows the variance is as bad as the mean: two of three chips fail to learn at all (0.25, 0.21), while the third---the chip seed used for the dose-response---reaches 0.46. Yield, not average accuracy, is the binding metric: the hope that slow weight adaptation absorbs static offsets does \emph{not} hold at realistic mismatch. At $10$\,mV the trajectory peaks early ($0.38$) then collapses to chance---the signature of offset-driven weight drift overwhelming the learning signal. Cell-resolved ablations at $\sigma{=}5$\,mV localize the damage: offsets confined to the 112 neuron/comparator devices alone are \emph{worse than the full chip} ($0.17$, flat---the gain-16 neurons amplify $5$\,mV to ${\sim}80$\,mV of activation shift); error-cell-only gives $0.53\!\to\!0.24$ (learn, then drift); update-cell-only gives $0.78\!\to\!0.55$ (a slow poison, not a blocker). Training-time input-noise augmentation is no rescue either: jitter of $0.3\sigma$ on the inputs leaves the clean baseline essentially unchanged (best epoch 0.85 vs 0.84) and a mismatched chip exactly where it was (0.45 vs 0.46)---the failure is structural offset-steering of learning, not a sharp-minimum effect that noise could regularize away. Partial hardening (neurons, or neurons+error cells, to $1$\,mV $\approx 25\times$ area) recovers only $0.45$/$0.56$---the damage is distributed; the synapse array's offsets are co-killers. Even chip-wide $\sigma=1$\,mV---which already implies large devices---halves the system (best epoch 0.56), so area scaling alone is impractical: the architecture requires offset-canceling cell variants (auto-zeroing / correlated double sampling, which solved exactly this problem in a companion workstream). We report this as a first-class result: any claim about analog learning hardware stands or falls here.

The rescue story, however, ends well---and the decisive experiment is a \emph{long} one. Short screens suggested rescue saturates around $0.45$; they were wrong about the ceiling because they truncated a slow climb. The combination of learnable biases, the sign-based update rule, and a learning-rate anneal-to-zero at the peak epoch, run to full length, turns the yield catastrophe around: across a five-chip Monte-Carlo at $\sigma{=}2$\,mV (the regime where two of three unaided chips learned \emph{nothing}), all five rescued chips learn, with best-epoch accuracies $\{0.72, 0.72, 0.83, 0.83, 0.82\}$, mean $\mathbf{0.78}$---93\% of the clean baseline ($0.84$), with zero new hardware: two existing cell types plus a controller schedule. Yield goes from roughly one-in-three to five-of-five. The honest deploy number is best-epoch (validation early-stop): some chips drift after their peak because the anneal was scheduled at a single global epoch rather than each chip's own peak, and per-chip freeze timing recovers the gap. Self-calibration through on-chip learning is therefore real but conditional---it requires parameters placed where offsets live (biases at every neuron input), an update rule whose own offset cross-section is small (sign-based), and a schedule that stops learning before offset-driven drift unwinds it. Three cautions from the same campaign: area-only hardening composes multiplicatively with biases (neuron upsizing alone $0.24$, biases alone $0.39$, together $0.51$ at screen length) but is \emph{chaotic} near the yield cliff (a strictly cleaner chip variant scored $0.22$); variance dominates means at the edge, so single-chip rescue numbers are anecdotes until replicated; and the anneal-at-peak control is general---it also converts a noise-augmented run that peaked at $0.87$ and crashed to chance into a stable $0.830$ (confirming noise augmentation as accuracy-neutral against the clean bias record of $0.860$).

\paragraph{The advantage of \emph{learning} survives moderate mismatch.} The catastrophe above is a depth-4, $C{=}4$ story; the ten-class deep pyramid (64--36--16, the rule-respecting net whose hidden layers learn in circuit, Section~\ref{sec:scaling}) is markedly more robust, and---the point that matters for the central claim---\emph{the advantage of training over freezing carries through}. On one chip ($\sim$1{,}400 series-gate $\Delta V_T$ sources), both halves of the trained-vs-frozen comparison degrade only gently, and \emph{equally}, all the way to $5$\,mV: frozen random features are essentially mismatch-immune (epoch-8 bijective accuracy $0.448\!\to\!0.452\!\to\!0.448\!\to\!0.436$ at $\sigma{=}0,1,2,5$\,mV---a trained linear readout simply absorbs the static per-feature offsets), and the in-circuit-\emph{trained} network tracks it ($0.524\!\to\!0.520\!\to\!0.520\!\to\!0.508$), so the trained-minus-frozen gap is preserved across the entire sweep ($+0.076,+0.068,+0.072,+0.072$) with no sign of the training-specific drift that sank the depth-4 net; the per-class prediction histograms are near-identical across $\sigma$. Two structural reasons distinguish this from the depth-4 collapse: one fewer stage compounds less per-stage offset amplification, and even a $5$\,mV offset is only a few-percent input-referred perturbation against $100$--$500$\,mV signal swings---too small to move many decisions in a network whose boundaries are already coarse at $0.52$. Learnable biases are accuracy-neutral here ($0.504$ at $1$\,mV), consistent with there being no large offsets to self-cancel at this $\sigma$. So the two mismatch results bracket the design space honestly: deep, high-accuracy in-circuit learning is offset-fragile and needs the bias/sign/anneal rescue above, while shallower moderate-accuracy learning is mismatch-tolerant and---crucially---keeps the value that \emph{training} adds over fixed random features. (One feature seed, replicated across three independent mismatch chips at $\sigma{=}2$\,mV: the trained network scores $0.520$ on all three while frozen gives $0.448/0.444$, so the $+0.07$ gap is preserved chip-to-chip with negligible cross-chip variance in this robust regime.)

\paragraph{Toward offset-lean update cells.} The mismatch ablations point to a concrete redesign. Since the amplitude-restoring backward path works on regenerated signs, the \emph{update} may not need a full four-quadrant analog multiply either: with one comparator per neuron regenerating $\mathrm{sign}(\eps)$ (amortized over its fan-in), the per-synapse update cell reduces to a V-to-I pair on $a$ plus a four-switch commutator steered by the sign---9 transistors (two of them static common-mode pull-ups) replacing \texttt{gprod}'s 15, with only \emph{two} offset-critical input devices instead of eight. We have characterized this cell at DC: its differential output current is cleanly odd in $a$ ($\pm6.5\,\mu$A full scale vs.\ \texttt{gprod}'s $\pm8.5\,\mu$A) and exactly mirrored under sign reversal. A full training run with the sign-update \emph{rule} (comparator feeding the existing update cells) validates it: best-epoch 0.83 versus 0.84 for the full four-quadrant multiply --- the analog error magnitude contributes almost nothing to the update. The cell therefore saves $\sim$12\% of system transistors while shrinking the update path's mismatch cross-section by $4\times$.
...
\item \textbf{The precision budget is uneven but global}: gain stages (neurons) are the most mismatch-critical components per device, yet hardening them alone does not rescue the system (Section~\ref{sec:mismatch}); rescue levers (area $\times$ biases) compose multiplicatively.
\item \textbf{Freeze at the peak.} Analog PC trajectories that peak and collapse (high gain, noise, mismatch) are uniformly stabilized by annealing the learning rate to zero at the peak epoch---a controller schedule, not hardware. The peak time must be measured per configuration; annealing early truncates learning instead.
\item \textbf{Cascade-gain budget.} Per-stage neuron gain must scale as $G_*^{1/\text{depth}}$ to keep total gain through the layer cascade inside a bounded stability band. Measured over four depths (rectifier net, fixed width, mirror-ratio gain in $\mu$m of output device): a one-hidden-layer net trains only at high gain ($\ge 800$) with no upper limit observed; the over-gain ceiling---above which the cascade over-drives into perfect anti-classification---then falls monotonically with depth ($>1600$, ${\sim}1600$, ${\sim}800$, ${\sim}400$--$800$ for one through four hidden layers). Shallow nets want the gain cranked; deep nets want it turned down. (An earlier reading of these data as a fan-in law was wrong---a controlled fan-in sweep leaves the boundary fixed; depth moves it.)
\item \textbf{Mismatch yield falls with depth.} The same cascade that compounds gain compounds device offsets: a one-hidden-layer network tolerates $\sigma_{V_T}\approx 2$\,mV, while adding a second hidden layer drops the yield to ${\sim}1/3$ at $1$\,mV (deaths are mismatch-fatal---weight-init restarts and lower gain do not recover them). Depth buys representational power but costs robustness, so under mismatch the architecture preference \emph{inverts}: shallow-and-wide beats deep-and-narrow. This sharpens the precision-budget law into a depth-dependent one.
\end{enumerate}
