...
\item \textbf{Gain is the unlock, and it obeys a width law.} The PMOS mirror ratio sets the activation gain. At gain ${\sim}1$ the rectifier network is no better than tanh; at gain ${\sim}5$ ($4\times$ mirror) a tiny 2--4--2 network solves \emph{all three} 2D benchmarks --- circles, rings, and the two-spirals problem that defeats single-hidden-layer tanh networks --- at $0.97$--$1.000$, several seeds pinned at $1.000$ with flat curves. The required per-cell gain scales \emph{inversely} with layer fan-in (a 2--8--4 network anti-locks at gain 5 for every init but solves all three tasks at $1.000$ with gain 2.6 plus a learning-rate anneal); gain ${\sim}10$ solves and then flips basins mid-training unless annealed.
...
Two partial-rescue results refine the picture. First, the rescue levers \emph{multiply}: on a dead chip ($0.25$ at $\sigma{=}2$\,mV), neuron-only upsizing ($4\times$ area on ${\sim}3\%$ of devices) recovers nothing by itself ($0.24$) and learnable biases alone recover $0.39$, but the two together reach $\mathbf{0.51}$ and still rising at the screen horizon---the bias can absorb what remains of a \emph{smaller} neuron offset, while neither mechanism suffices alone. Second, rescue outcomes at the failure edge are \emph{chaotic}: making the same chip strictly cleaner (additionally halving the error-cell offsets) yielded $0.22$---worse than the dirtier configuration. Near the yield cliff, variance dominates every mean; we therefore report rescue numbers as single-chip anecdotes, not expectations. Finally, the universal peak-then-collapse signature has a universal control: annealing the learning rate to zero at the peak epoch (a pure controller schedule, no new hardware) converts every collapsing trajectory we tested into a flat plateau---e.g.\ a noise-augmented run that peaked at $0.87$ and crashed to chance instead locks at $0.830$ to the end of a $2\times$ horizon. With that control in place, noise augmentation is confirmed accuracy-neutral at full horizon (locked $0.83$ vs the bias record $0.860$).
