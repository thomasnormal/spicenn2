...
def gen():
    ...
    if TASK=="circles":
        ...
        if int(os.environ.get("CIF64","0")):
            d=np.load(os.path.join(os.path.dirname(os.path.abspath(__file__)),"data/cifar_c64.npz"))
        else:
...
def gen_deck():
    ...
    AFL=float(os.environ.get("AFLOOR","0.6")); VBBK=float(os.environ.get("VBBK","0.35")); gBLH=float(GBLH); gBLO=float(GBLO)   # VBBK lowered 0.45->0.35: sweep optimum ~0.32 (keeps backward magnitude; below ~0.28 under-drives)
