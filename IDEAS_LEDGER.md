...
### 4-seed graded corroboration: gph_s4=0.488 -> 4-seed graded {0.524,0.500,0.464,0.488} mean 0.494+/-0.022, STILL non-overlapping w/ frozen (all >=0.464 > frozen max 0.448). Confirms the 3-seed 0.496 in paper (4-seed 0.494 identical within rounding). Result rock-solid.

### NEURON-CELL fidelity search (2026-06-29, workflow + 2 new proxies)
Built reusable tooling: experiments/char_neuron.py (Spectre DC transfer characterizer: gain/swing/tanh_rms/sat90/headroom vs ideal tanh; validated baseline dneuron = 0.535*tanh(17x), rms 2.8%, matches paper), predict_acc.py (FROZEN-feature ridge proxy), predict_trained.py (TRAINED-PC proxy via train_faithful + measured curve as activation). Workflow neuron-fidelity-search designed+characterized 15 distinct cells (degen/cascode/xcoupled/diode/resistor-load/two-stage/mirror/folded/bump/wide-gm); all build at 1V.
KEY METHODOLOGY: the FROZEN-feature ridge proxy is ARTIFACTUAL -> rewards LINEARITY (linear classifier wins; its tanh ref hardwired at over-saturated g16). Adversarial verify caught it (pure identity 0.769 matches every "winner"). The TRAINED proxy INVERTS it (validated): near-linear curve 73->24 when hidden TRAINS (deep-linear collapses); baseline-curve=tanh=72. So rank neurons by the TRAINED proxy, NOT frozen.
TRAINED-proxy ranking (EP40 3-seed, baseline tanh~73): widegm6(sharper g27)=75.1 (+1.7, ONLY cell beating tanh) > baseline 72.3 > tailvary 71 > degenlight(g12) 63 > gentler cells 30-46 > bumpeven 9. MONOTONE IN GAIN (g12->17->27 = 63->72->75). => "better forward features" = MORE neuron gain (sharper tanh), NOT gentler. widegm6 = just DNW wider (wider diff pair).
CAVEATS: +1.7 is ~1sd (marginal); and wider-gm/DNW-up was REFUTED in-circuit in the OLD regime (analog-gap memory: DNW 0.367->0.311, gain coupled to settling/saturation and failed). The proxy is the IDEAL trainer (g=0 synapse, ideal backward) -> isolates activation; in-circuit settling/saturation punishes high gain. DECIDER = a Spectre TRAINING run with DNW up in the CURRENT trained-deep recipe (regime differs from the old DNW test). NEXT: gain-sweep proxy optimum -> launch gph_dnw<W> Spectre when a slot frees, compare to 0.494.

### Neuron gain sweep (trained proxy, EP60 3-seed) — clean INVERTED-U optimum: g12=71.1 / g17(baseline)=76.8 / g24=80.0(+3.2,sd1.2 PEAK) / g34=79.7(+2.9) / g48=76.7 / g68=72.7(-4.1). Baseline neuron (g~17) sits BELOW the optimum; proxy says moderate gain UP (g~24-34, ~DNW 400-800u) gives +3 in the IDEAL trainer. gph_dnw600 (DNW=600u ~ g28-30) is well-placed near the peak. STRONGER than the +1.7 single-cell read. BUT still the IDEAL trainer (no synapse-sat/settling); the in-circuit DNW refutation (old regime) means gph_dnw600 (real Spectre, current regime) is the decider. If gph_dnw600 > 0.494 -> bracket DNW400/DNW800 + 3-seed.
### Extension probe reads (vs trained-deep baseline 0.494 mean / seed-1 0.524): gph_bias (BIASW=1 learnable bias) epoch-8 = 0.512 ~= no-bias seed-1 0.524 -> NEUTRAL at C=10 (paper's +0.02 bias gain was C=4-specific). gph_long epoch-8=0.524 (=seed-1, determinism; longer-training payoff at ep16/ep24 pending). STILL PENDING: gph_dnw600 (neuron gain, key) ~1.5hr, gph_d4 (depth) ~3hr, gph_long ep16/24.
### gph_dnw600 (DNW=600u higher neuron gain) epoch-8 = 0.516 ~= baseline seed-1 0.524 -> NEUTRAL in-circuit. The proxy gain-signal (+3.2 ideal trainer) does NOT transfer to the real trained-deep circuit -> CONFIRMS the old in-circuit DNW/gain refutation (settling/saturation/cascade-budget erase the gain benefit). NEURON-GAIN LEVER EXHAUSTED; baseline tanh near-optimal. Tally vs 0.494: gph_bias NEUTRAL(0.512), gph_dnw600 NEUTRAL(0.516). Last live lever = gph_long longer-training (ep16/ep24).

### ★ CONSOLIDATION: trained-deep C=10 plateau = 0.494 (mean) / ~0.52 (seed-1), all "go higher" levers EXHAUSTED in-circuit (2026-06-29)
gph_long ep16=0.520 (=ep8 0.524 -> longer training PLATEAUS, neutral). Full tally vs baseline 0.494/seed1 0.524: BIASES neutral (gph_bias 0.512), NEURON-GAIN neutral (gph_dnw600 DNW=600u 0.516 -- proxy +3.2 did NOT transfer, confirms old DNW refutation), LONGER-TRAINING neutral (plateau), DEPTH-4 killed (I/O+speculative). => 0.494 is the device-fidelity-bound in-circuit-trained C=10 plateau; no cell/software lever exceeds it (consistent with [[analog-gap-diagnosis]]: residual to ideal surrogate = analog DEVICE-CELL fidelity, not algorithm). NEURON-FIDELITY track CLOSED: baseline tanh is near-optimal; the trained-net-correct screen (predict_trained.py) shows only sharper-gain helps in the IDEAL trainer but it does not survive in-circuit. CORE GOAL (training beats frozen, 0.496 vs 0.429, 4-seed) STANDS, in paper. Next real lever would be HARDWARE (lower-noise/better-matched devices, higher supply), not software -- needs user direction.

### MISMATCH VALIDATION launched (user-directed 2026-06-29): does training-beats-frozen survive device mismatch? MMVT=per-device VT offset sigma (V), MMSEED=chip. Clean baseline: trained 0.494 vs frozen 0.429 (gap +0.067). First look at sigma=1mV, 1 chip (MMSEED=0), WSEED=1, 3 configs: frz_mm1 (NOHID frozen), gph_mm1 (graded trained), gph_mm1b (graded trained + BIASW=1 self-calibration). Q: does the trained>frozen GAP hold under mismatch, and do biases rescue (paper: biases are the strongest self-cal lever under mismatch)? Paper mismatch is C=4 depth-4; this is the C=10 trained-deep regime. 1 chip = first look; >=3 chips (MMSEED 0,1,2) for robustness if gap survives (variance dominates at the cliff).
### MISMATCH (1mV, MMSEED=0): frz_mm1 (frozen+mismatch) epoch-8 = 0.452 ~= frozen-clean 0.448 -> FROZEN ROBUST to 1mV mismatch (trained readout absorbs static device offsets). Now need gph_mm1 (trained+mismatch) vs trained-clean 0.494: does TRAINING hold up (paper warns trained suffers offset-driven drift)? gph_mm1b tests bias self-cal. NOTE 1mV barely touches frozen -> may need 2mV to see a differentiating/catastrophic effect (paper catastrophe was 2mV+ at depth-4).
### MISMATCH 1mV (MMSEED=0, seed-1) RESULT: trained+mm gph_mm1=0.520 (~= clean 0.524) ; frozen+mm 0.452 (~= clean 0.448). GAP PRESERVED under 1mV mismatch: +0.068 (vs clean +0.076). => in-circuit-learning advantage (training beats frozen) FULLY SURVIVES 1mV device mismatch; BOTH nets robust (readout absorbs offsets for frozen; trained doesn't drift at 1mV at this depth, unlike paper's depth-4 catastrophe). Escalating to 2mV to find the stress point.
### MISMATCH 1mV COMPLETE (seed-1 MMSEED=0): frozen 0.452, trained 0.520, trained+bias 0.504 (all ~= clean). GAP +0.068 preserved. Biases NEUTRAL at 1mV (no real offsets to self-cal at this mild sigma; paper's bias-rescue was at 2mV+). => in-circuit learning FULLY ROBUST to 1mV mismatch at C=10 depth-3. Awaiting 2mV stress (frz_mm2, gph_mm2) ~2-3hr.
### MISMATCH 2mV: frz_mm2 (frozen 2mV) epoch-8 = 0.448 = frozen-clean -> FROZEN ROBUST even at 2mV (flat: clean 0.448 / 1mV 0.452 / 2mV 0.448; readout absorbs static offsets at any sigma). KEY remaining: gph_mm2 (trained 2mV) -> does TRAINING hold ~0.52 (robust, gap preserved) or DRIFT toward/below frozen 0.448 (training-specific sensitivity, paper depth-4 catastrophe regime)? ~1.5hr.

### ★ MISMATCH ROBUSTNESS COMPLETE (1-2mV, seed-1 MMSEED=0) — in-circuit learning SURVIVES realistic mismatch:
  sigma:    clean   1mV     2mV
  frozen:   0.448   0.452   0.448  (FLAT -- trained readout absorbs static offsets at any sigma)
...
### ONN PROTOTYPE — PROGRAMMABLE coupling (weight-controlled synchronization, 2026-06-30)
Extended experiments/onn.py with COUPLE=tgate: coupling = transmission gate (NMOS gate=VW + PMOS gate=1-VW) -> conductance K_ij = f(VW), a WEIGHT VOLTAGE (storable on a cap -> learnable). Demonstrates programmable synchronization in real Spectre CMOS:
  - 2-osc (detuned 5.5%): sharp WEIGHT THRESHOLD for locking — VW<=0.36 unlocked, VW>=0.39 LOCKED (transition ~0.37). The weight turns coupling on/off.
  - N=5 collective: VW=0.25 (weight OFF) -> 14.9% spread unlocked; VW=0.80 (weight ON) -> 0.0% LOCKED (all 5 synchronized). Weight voltage programmatically controls collective sync.
BRIDGE TO LEARNABLE ONN COMPLETE (at the inference/programmability level): K_ij is now a stored gate voltage. The remaining open piece = an IN-CIRCUIT LEARNING RULE for the weights (e.g. Hebbian-on-phase-coherence: dK_ij ~ cos(theta_i - theta_j) via a phase-detector/mixer) — the on-substrate-learning thesis applied to oscillators (vs UN-0 / ONN literature training K off-chip). onn.py params: N, COUPLE=res|tgate, VW, RC, DET, TSTOP.

### ONN: IN-CIRCUIT PHASE DETECTOR (learning-signal primitive, 2026-06-30)
Added PD=1 to experiments/onn.py: BUFFERED CMOS XOR(o0,o1)+RC lowpass -> DC encodes phase relationship. NON-INVASIVE (buffer = inverter pair, only gate-cap load on osc nodes; verified: uncoupled pair stays unlocked 5.6% with PD attached). Output: UNLOCKED (beating) -> PD DC 0.610V; LOCKED in-phase (VW=0.80) -> PD DC 0.261V. The circuit MEASURES phase coherence in-situ = the learning signal.
LEARNABLE-ONN STATUS: all primitives now exist in real Spectre CMOS — (1) oscillators (omega_i via starve bias), (2) PROGRAMMABLE coupling (K_ij via weight voltage VW, storable on cap), (3) PHASE DETECTOR (reads coherence -> learning signal). REMAINING for a closed in-circuit learning loop: an update integrator that drives VW from the PD signal (Hebbian: dK_ij ~ coherence(theta_i,theta_j)) + a task (associative memory / Ising). That closed loop = the on-substrate-learning thesis applied to oscillators (vs UN-0/ONN-lit training K off-chip). experiments/onn.py params: N, COUPLE=res|tgate, VW, RC, DET, PD, TSTOP.

### ★★★ ONN CAPSTONE: CLOSED-LOOP IN-CIRCUIT LEARNING on the oscillator substrate (2026-06-30)
experiments/onn_learn.py: 2 detuned ring oscillators, coupled by an NMOS whose gate = WEIGHT node vw (a 4p cap). In-circuit phase detector (buffered XOR+lowpass -> pdf) reads coherence; a 5T OTA integrates (pdf - vtarget) onto the weight cap. The whole loop is transistors -- NO external optimizer. The coupling weight LEARNS to synchronize:
  LEARNING ON (VBU=0.45): vw 0.180 -> 0.407V (self-rose), oscillators -> LOCKED 0.9%.
  CONTROL frozen weight (VBU=0.05): vw 0.180 -> 0.180V (no update), oscillators STAY UNLOCKED 4.5%.
Same start; learning -> lock, frozen -> no lock => the SYNCHRONIZATION IS LEARNED IN-CIRCUIT (control rules out initial conditions). This closes the on-substrate-learning thesis on a DIFFERENT physical substrate (Kuramoto oscillators), complementary to the PC net: UN-0 and the ONN literature train K off-chip; here measure-phase -> update-weight is all transistors. Full ONN stack now built+pushed: ring oscillators -> coupling/sync (Arnold tongue, N=5) -> programmable weight (tgate) -> phase detector -> CLOSED LEARNING LOOP. Next extensions: a real TASK (associative memory = store/recall phase patterns; or Ising/optimization) and N>2 weight matrices; phase computing suits these, not feedforward classification.
